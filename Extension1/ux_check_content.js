				...
				if ($('#he_tray').length == 0){
					...
					chrome.storage.local.get(['ux_check_last_opened', 'purple_upsell_shown'], function(result){
						...
						chrome.storage.local.get('he_settings_saved_data', function(result){ 
							...
							console.log(ux_check_last_opened);
							if(typeof ux_check_last_opened !== 'undefined'){
								console.log(purple_upsell_shown);
								if (typeof purple_upsell_shown === 'undefined'){
									...
									// Show dialog
									// var videoUrl = chrome.extension.getURL('bigpicture.mp4');
									// var restartUrl = chrome.extension.getURL('restart.png');
									// $('#add').append(	'<div id="purple_dialog_background">'+
									// 						'<div id="purple_dialog">'+
									// 							'<p>Hey there,</p>' + 
									// 							"<p>I'm Chris - the creator of UX Check. Just want to share something that I'm <b>so excited</b> about! For the past two years, I've been working with a great team of developers to build a new design tool called Purple. It lets you create all-in-one boards that contain lists, documents, presentations, designs, and prototypes, so all of your project work is in one place. And we integrate with Google Docs, Sketch, Invision, Marvel, Figma, Adobe XD, Atomic, and Airtable, so you don't have to give up your existing tools.</p>" +
									// 							"<p>Purple is now open as a free public beta. Go check it out!</p>" + 
									// 							'<p>Chris</p>'+
									...
									// 							'<div style="width:100%;display:flex;"><a id="purple_CTA_button" target="_blank" href="http://www.purple.li?utm_source=uxcheck">Purple.pm - free beta</a></div>' +
